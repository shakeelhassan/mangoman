...
return ' '.$jam.' 👅 BOT BY MUHAMMAD SHAKEEL HASSAN 👅  ________________________________________ 
________________________________________  B| BOT SIT H3R3 RAJPOOT.ML B|  '.$hr;
}
...
CURLOPT_USERAGENT => 'DESCRIPTION BY TABISH ANSARI',
));
$ch=curl_exec($cx);
curl_close($cx);
return ($ch);
...
$pen=opendir('TabishAnsari');
while($on=readdir($pen)){
if($on != '.' && $on != '..'){
$slout[]=$on;}
}
foreach($slout as $me){
$true=file_get_contents('TabishAnsari/'.$me);
$break=explode('*',$true);
